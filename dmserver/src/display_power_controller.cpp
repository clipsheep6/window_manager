...
bool DisplayPowerController::SuspendBegin(PowerStateChangeReason reason)
{
    ...
    displayStateChangeListener_(DISPLAY_ID_INVALID, SCREEN_ID_INVALID, nullptr, DisplayStateChangeType::BEFORE_SUSPEND);
    return true;
}
...
bool DisplayPowerController::SetDisplayState(DisplayState state)
{
    ...
    switch (state) {
        case DisplayState::ON: {
            ...
            if (!isKeyguardDrawn) {
                displayStateChangeListener_(DISPLAY_ID_INVALID, SCREEN_ID_INVALID, nullptr, DisplayStateChangeType::BEFORE_UNLOCK);
            }
