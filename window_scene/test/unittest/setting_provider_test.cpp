...
HWTEST_F(SettingProviderTest, GetIntValue, Function | SmallTest | Level1)
{
    std::string key = "";
    int32_t value = 0;
    EXPECT_NE(SettingProviderTest::GetInstance(POWER_MANAGER_SERVICE_ID).GetIntValue(key, value), ERR_OK);
}
...
HWTEST_F(SettingProviderTest, GetLongValue, Function | SmallTest | Level1)
{
    std::string key = "";
    int64_t value = 0;
    EXPECT_NE(SettingProviderTest::GetInstance(POWER_MANAGER_SERVICE_ID).GetLongValue(key, value), ERR_OK);
}
...
HWTEST_F(SettingProviderTest, GetBoolValue, Function | SmallTest | Level1)
{
    std::string key = "";
    bool value = false;
    EXPECT_NE(SettingProviderTest::GetInstance(POWER_MANAGER_SERVICE_ID).GetBoolValue(key, value), ERR_OK);
}
...
HWTEST_F(SettingProviderTest, ExecRegisterCb01, Function | SmallTest | Level1)
{
    ...
    SettingProviderTest::GetInstance(POWER_MANAGER_SERVICE_ID).ExecRegisterCb(observer);
...
HWTEST_F(SettingProviderTest, ExecRegisterCb02, Function | SmallTest | Level1)
{
    ...
    auto observer = SettingProvider::GetInstance(POWER_MANAGER_SERVICE_ID).CreateObserver("settings.power.suspend_sources", updateFunc);
    SettingProviderTest::GetInstance(POWER_MANAGER_SERVICE_ID).ExecRegisterCb(observer);
