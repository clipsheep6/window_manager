...
{
    GTEST_LOG_(INFO) << static_cast<int32_t>(WindowMode::WINDOW_MODE_FLOATING);
    GTEST_LOG_(INFO) << static_cast<int32_t>(WindowMode::WINDOW_MODE_FULLSCREEN);
    std::string xmlStr = "<?xml version='1.0' encoding=\"utf-8\"?>"
        "<Configs>"
    ...
    SceneSessionManager* sceneSessionManager = new SceneSessionManager();
    sceneSessionManager->ConfigWindowSceneXml();
    ASSERT_EQ(sceneSessionManager->systemConfig_.defaultWindowMode_,
...
{
    GTEST_LOG_(INFO) << static_cast<int32_t>(MaximizeMode::MODE_AVOID_SYSTEM_BAR);
    GTEST_LOG_(INFO) << static_cast<int32_t>(MaximizeMode::MODE_FULL_FILL);
    std::string xmlStr = "<?xml version='1.0' encoding=\"utf-8\"?>"
        "<Configs>"
...
HWTEST_F(SceneSessionManagerTest, ConfigWindowSceneXml03, Function | SmallTest | Level3)
{
    ...
    SceneSessionManager* sceneSessionManager = new SceneSessionManager();
    sceneSessionManager->ConfigWindowSceneXml();
    ASSERT_EQ(sceneSessionManager->systemConfig_.maxFloatingWindowSize_,
...
HWTEST_F(SceneSessionManagerTest, ConfigWindowAnimation01, Function | SmallTest | Level3)
{
    ...
    ASSERT_EQ(sceneSessionManager->appWindowSceneConfig_.windowAnimation_.scaleX_, 0.7);
    ASSERT_EQ(sceneSessionManager->appWindowSceneConfig_.windowAnimation_.scaleY_, 0.7);
...
HWTEST_F(SceneSessionManagerTest, ConfigSnapshotScale01, Function | SmallTest | Level3)
{
    ...
    ASSERT_EQ(sceneSessionManager->snapshotScale_, 0.7);
}
