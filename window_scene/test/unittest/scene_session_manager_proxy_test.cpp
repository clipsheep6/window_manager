...
HWTEST_F(sceneSessionManagerProxyTest, GetTopWindowId, Function | SmallTest | Level2)
{
    ...
    uint32_t topWinId = 1;
    ASSERT_EQ(WMError::WM_ERROR_IPC_FAILED, sceneSessionManagerProxy_->GetTopWindowId(0, topWinId));
    sceneSessionManagerProxy_ = nullptr;
}
...
HWTEST_F(sceneSessionManagerProxyTest, GetVisibilityWindowInfo, Function | SmallTest | Level2)
{
    ...
    ASSERT_EQ(WMError::WM_ERROR_IPC_FAILED, sceneSessionManagerProxy_->GetVisibilityWindowInfo(infos));
    sceneSessionManagerProxy_ = nullptr;
}
 ...
 * @tc.type: FUNC
 */
HWTEST_F(sceneSessionManagerProxyTest, ShiftAppWindowFocus, Function | SmallTest | Level2)
{
    sptr<IRemoteObject> iRemoteObjectMocker = new (std::nothrow) IRemoteObjectMocker();
    sptr<SceneSessionManagerProxy> sceneSessionManagerProxy_ = new SceneSessionManagerProxy(iRemoteObjectMocker);

    ASSERT_EQ(WSError::WS_OK, sceneSessionManagerProxy_->ShiftAppWindowFocus(0, 0));
    sceneSessionManagerProxy_ = nullptr;
}
}
}
