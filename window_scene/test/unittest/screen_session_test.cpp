 ...
 * @tc.type: FUNC
 */
HWTEST_F(ScreenSessionTest, SetScreenColorSpace01, Function | SmallTest | Level2)
{
...
/**
 * @tc.name: RegisterScreenChangeListener01
 * @tc.desc: normal function
 * @tc.type: FUNC
 */
...
/**
 * @tc.name: RegisterScreenChangeListener02
 * @tc.desc: normal function
 * @tc.type: FUNC
 */
...
{
    GTEST_LOG_(INFO) << "ScreenSessionTest: Connect start";
    sptr<ScreenSession> session = new(std::nothrow) ScreenSession();
    session->Connect();
    ASSERT_EQ(session->screenState_, ScreenState::CONNECTION);
    GTEST_LOG_(INFO) << "ScreenSessionTest: Connect end";
}
...
HWTEST_F(ScreenSessionTest, Disconnect02, Function | SmallTest | Level2)
{
    ...
    sptr<ScreenSession> session = new(std::nothrow) ScreenSession();
    session->Disconnect();
    ASSERT_EQ(session->screenState_, ScreenState::DISCONNECTION);
