 ...
 * @tc.type: FUNC
 */
HWTEST_F(ScreenSessionTest, GetOrientation, Function | SmallTest | Level2)
{
    GTEST_LOG_(INFO) << "ScreenSessionTest: GetOrientation start";
    sptr<ScreenSession> session = new(std::nothrow) ScreenSession();
    Orientation res = session->GetOrientation();
    ASSERT_EQ(res, session->property_.GetOrientation());
...
HWTEST_F(ScreenSessionTest, SetOrientation, Function | SmallTest | Level2)
{
    ...
    session->SetOrientation(res);
    session->property_.SetOrientation(orientation);
