 ...
 * @tc.desc: SceneSessionManagerSupplementTest HandleCastScreenDisConnection
 * @tc.type: FUNC
 */
HWTEST_F(SceneSessionManagerSupplementTest, HandleCastScreenDisConnection,
    Function | SmallTest | Level3)
{
    int res = 0;
    SessionInfo info;
    info.bundleName_ = "test1";
    info.abilityName_ = "test2";
    info.screenId_ = ScreenSessionManagerClient::GetInstance().GetDefaultScreenId();
    sptr<SceneSession> sceneSession;
    sceneSession = new (std::nothrow) SceneSession(info, nullptr);
    ssm_->HandleCastScreenDisConnection(sceneSession);
    ssm_->HandleCastScreenDisConnection(sceneSession);
    info.screenId_ = info.screenId_ + 1;
    sptr<SceneSession> sceneSession2 = new (std::nothrow) SceneSession(info, nullptr);
    ssm_->HandleCastScreenDisConnection(sceneSession2);
    ...
    ssm_->DestroySpecificSession(object);
    ssm_->remoteObjectMap_.insert({object, 1});
    ssm_->DestroySpecificSession(object);
...
HWTEST_F(SceneSessionManagerSupplementTest, RecoverAndConnectSpecificSession, Function | SmallTest | Level3)
{
    ...
    sptr<ISession> session;
    sptr<IRemoteObject> token;
    ssm_->RecoverAndConnectSpecificSession(sessionStage, eventChannel, node, property, session, token);
    property = new WindowSessionProperty();
    ssm_->RecoverAndConnectSpecificSession(sessionStage, eventChannel, node, property, session, token);
 ...
 * @tc.type: FUNC
*/
HWTEST_F(SceneSessionManagerSupplementTest, GetResourceManager, Function | SmallTest | Level3)
{
    int ret = 0;
    AppExecFwk::AbilityInfo abilityInfo;
    ssm_->GetResourceManager(abilityInfo);
    std::string path = "aa";
    uint32_t color = 888;
    ssm_->GetStartupPageFromResource(abilityInfo, path, color);
    SessionInfo sessionInfo;
    abilityInfo.moduleName = "m1";
    abilityInfo.name = "n1";
    abilityInfo.bundleName = "b1";
    ssm_->GetStartupPage(sessionInfo, path, color);
    ssm_->GetStartingWindowInfoFromCache(sessionInfo, path, color);
    ssm_->CacheStartingWindowInfo(abilityInfo, path, color);
    sessionInfo.abilityName_ = "a1";
    sessionInfo.bundleName_ = "b1";
    StartingWindowInfo startWindowInfo;
    std::map<std::string, StartingWindowInfo> infos;
    infos.insert({"a1", startWindowInfo});
    ssm_->OnBundleUpdated("b1", 0);
    ssm_->startingWindowMap_.insert({"b1", infos});
    ssm_->GetStartingWindowInfoFromCache(sessionInfo, path, color);
    ssm_->CacheStartingWindowInfo(abilityInfo, path, color);
    sessionInfo.moduleName_ = "m1";
    infos.insert({"m1a1", startWindowInfo});
    ssm_->startingWindowMap_.insert({"b1", infos});
    ssm_->GetStartingWindowInfoFromCache(sessionInfo, path, color);
    ssm_->CacheStartingWindowInfo(abilityInfo, path, color);
    ssm_->OnBundleUpdated("b1", 0);
