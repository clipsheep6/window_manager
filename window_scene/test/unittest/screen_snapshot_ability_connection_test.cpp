...
HWTEST_F(ScreenSnapshotAbilityConnectionTest, OnAbilityConnectDone, Function | SmallTest | Level1)
{
    ...
    Want want;
    want.SetElementName("com.huawei.ohos.screenshot", "com.huawei.ohos.screenshot.ServiceExtAbility");
    std::string identity = IPCSkeleton::ResetCallingIdentity();
    auto resConnect = ExtensionManagerClient::GetInstance().ConnectServiceExtensionAbility(
    ...
    ASSERT_EQ(resConnect, ERR_OK);
    {
        ...
        static_cast<void>(connectedCv_wait_for(lock, std::chrono::milliseconds(EXTENSION_CONNECT_OUT_TIME)));
    }
    EXPECT_EQ(abilityConnection->IsAbilityConnected(), true);
    auto resDisconnect = ExtensionManagerClient::GetInstance().DisconnectAbility(abilityConnection);
...
HWTEST_F(ScreenSnapshotAbilityConnectionTest, SendMessageSync, Function | SmallTest | Level1)
{
    ...
    abilityConnection->SendMessageSync(TRANS_CODE_CONNECTION, data, reply);
