...
WSError SessionStageProxy::TransferAccessibilityEvent(const Accessibility::AccessibilityEventInfo& info,
    const std::vector<int32_t>& uiExtensionIdLevelVec)
{
    WLOGFI("TransferAccessibilityEvent begin");
    MessageParcel data;
    MessageParcel reply;
    MessageOption option(MessageOption::TF_ASYNC);
    if (!data.WriteInterfaceToken(GetDescriptor())) {
    ...
    WLOGFI("TransferAccessibilityEvent end");
    return WSError::WS_OK;
}
} // namespace OHOS::Rosen
