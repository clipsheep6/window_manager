...
AvoidArea ExtensionSession::GetAvoidAreaByType(AvoidAreaType type)
{
    ...
    if (extSessionEventCallback_ != nullptr && extSessionEventCallback_->notifyGetAvoidByTypeFunc_ != nullptr) {
        avoidArea = extSessionEventCallback_->notifyGetAvoidByTypeFunc_(type);
    }
    return avoidArea;
}
