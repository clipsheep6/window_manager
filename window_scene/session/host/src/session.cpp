...
{
    WLOGFI("MR031 NotifyTransferAccessibilityEvent begin");
    auto lifecycleListeners = GetListeners<ILifecycleListener>();
    WLOGFI("MR031 NotifyTransferAccessibilityEvent  lifecycleListeners.size:%{public}d", lifecycleListeners.size());
    for (auto& listener : lifecycleListeners) {
        if (!listener.expired()) {
            WLOGFI("MR031 NotifyTransferAccessibilityEvent ---1");
            listener.lock()->OnAccessibilityEvent(info, uiExtensionIdLevelVec);
        }
    }
    WLOGFI("MR031 NotifyTransferAccessibilityEvent end");
}
