...
{
    if (moveDragProperty_.originalRect_.width_ == originalRect.width_) {
        return;
    }
    ...
    WLOGI("original rect [%{public}d, %{public}d, %{public}d, %{public}d]", posX, 0,
    ...
    const WSRect& originalRect)
{
    ...
    ResetOriginalPositionWhenFullScreenToFloating(originaRect);
...
void MoveDragController::ProcessStartMovePositionFunc()
{
    ...
    if (startMovePositionFunc_) {
        WLOGFI("ProcessStartMovePositionFunc start, xPosition: %{public}u, yPosition: %{public}b",
