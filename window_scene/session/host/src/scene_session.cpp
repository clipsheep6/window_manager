...
WSError SceneSession::OnLayoutFullScreen(bool status)
{
    WLOGFD("SceneSession OnLayoutFullScreen status:%{public}d", static_cast<int32_t>(status));
    if (sessionChangeCallback_ != nullptr && sessionChangeCallback_->OnLayoutFullScreen_) {
        sessionChangeCallback_->OnLayoutFullScreen_(status);
    }
    return WSError::WS_OK;
}
