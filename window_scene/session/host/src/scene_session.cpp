    ...
    const std::string& identityToken)
{
    ...
    TLOGI(WmsLogTag::WMS_LIFE,
        "SceneSession::Connect: %{public}s, identityToken: %{public}s, bundleName: %{public}s",
        clientIdentityToken_.c_str(), identityToken_.c_str(), GetSessionInfo().bundleName_.c_str());
...
WSError SceneSession::Foreground(sptr<WindowSessionProperty> property, bool isFromClient)
{
    ...
    if (isFromClient) {
        ...
        TLOGI(WmsLogTag::WMS_LIFE,
            "Foreground connectedClientPid_: %{public}d, callingPid: %{public}d, bundleName: %{public}s",
...
WSError SceneSession::Background(bool isFromClient)
{
    if (isFromClient) {
        int32_t callingPid = IPCSkeleton::GetCallingPid();
        TLOGI(WmsLogTag::WMS_LIFE,
            "Background connectedClientPid_: %{public}d, callingPid: %{public}d, bundleName: %{public}s",
            connectedClientPid_, callingPid, GetSessionInfo().bundleName_.c_str());
        if (callingPid != 0 && callingPid != connectedClientPid_) {
            TLOGW(WmsLogTag::WMS_LIFE,
...
WSError SceneSession::Disconnect(bool isFromClient)
{
    if (isFromClient) {
        int32_t callingPid = IPCSkeleton::GetCallingPid();
        TLOGI(WmsLogTag::WMS_LIFE,
            "Disconnect connectedClientPid_: %{public}d, callingPid: %{public}d, bundleName: %{public}s",
            connectedClientPid_, callingPid, GetSessionInfo().bundleName_.c_str());
        if (callingPid != 0 && callingPid != connectedClientPid_) {
            TLOGW(WmsLogTag::WMS_LIFE,
...
void SetClientIdentityToken(std::string& iToken)
{
