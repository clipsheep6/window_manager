...
public:
    ...
    WSError SetCallingWindowId(uint32_t callingWindowId) override;
    uint32_t GetCallingWindowId() override;

    void GetKeyboardSessionRect(WSRect &keyboardSessionRect);
    bool IsStatusBarVisible(const sptr<SceneSession>& session);
    int32_t GetStatusBarHeight();

private:
    std::shared_mutex keyboardSessionMapMutex_;
