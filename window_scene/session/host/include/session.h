...
public:
    ...
    virtual std::vector<Rect> GetTouchHotAreas() const { return std::vector<Rect>(); }
    virtual void SetTouchHotAreas(const std::vector<Rect>& touchHotAreas);
    void SetVpr(float vpr) { vpr_ = vpr; }
    ...
    bool operator!=(const Session* session) const { return !this->operator==(session); }
    ...
    virtual int32_t GetCustomDecorHeight() { return 0; };
