...
public:
    ...
    virtual WMError Connect(const sptr<ISessionStage>& sessionStage, const sptr<IWindowEventChannel>& eventChannel,
        const std::shared_ptr<RSSurfaceNode>& surfaceNode, uint64_t& persistentId,
        sptr<WindowSessionProperty> property = nullptr) = 0;
    virtual WMError Foreground() = 0;
    virtual WMError Background() = 0;
    virtual WMError Disconnect() = 0;
    virtual WMError PendingSessionActivation(const SessionInfo& info) = 0;
