...
const unsigned int XCOLLIE_TIMEOUT_30S = 30;
...
{
    DmsXcollie dmsXcollie("DMS:RegisterDisplayManagerAgent", XCOLLIE_TIMEOUT_30S);
...
{
    DmsXcollie dmsXcollie("DMS:GetDefaultDisplayInfo", XCOLLIE_TIMEOUT_30S);
    GetDefaultScreenId();
    sptr<ScreenSession> screenSession = GetScreenSession(defaultScreenId_);
...
{
    DmsXcollie dmsXcollie("DMS:GetDisplayInfoById", XCOLLIE_TIMEOUT_30S);
...
{
    DmsXcollie dmsXcollie("DMS:GetScreenInfoById", XCOLLIE_TIMEOUT_30S);
...
{
    DmsXcollie dmsXcollie("DMS:GetDensityInCurResolution", XCOLLIE_TIMEOUT_30S);
    sptr<ScreenSession> screenSession = GetScreenSession(screenId);
    if (screenSession == nullptr) {
...
{
    DmsXcollie dmsXcollie("DMS:CreateScreenProperty", XCOLLIE_TIMEOUT_10S);
...
{
    DmsXcollie dmsXcollie("DMS:UpdateScreenRotationProperty", XCOLLIE_TIMEOUT_30S);
    if (!SessionPermission::IsSystemCalling() && !SessionPermission::IsStartByHdcd()) {
...
{
    DmsXcollie dmsXcollie("DMS:GetFoldDisplayMode", XCOLLIE_TIMEOUT_30S);
    if (!g_foldScreenFlag) {
...
{
    DmsXcollie dmsXcollie("DMS:IsFoldable", XCOLLIE_TIMEOUT_30S);
    if (!g_foldScreenFlag) {
        return false;
    }
    if (foldScreenController_ == nullptr) {
...
{
    DmsXcollie dmsXcollie("DMS:GetFoldStatus", XCOLLIE_TIMEOUT_30S);
    if (!g_foldScreenFlag) {
...
{
    DmsXcollie dmsXcollie("DMS:SetClient", XCOLLIE_TIMEOUT_10S);
    if (!SessionPermission::IsSystemCalling()) {
        TLOGE(WmsLogTag::DMS, "permission denied, calling clientName: %{public}s, calling pid: %{public}d",
            SysCapUtil::GetClientName().c_str(), IPCSkeleton::GetCallingPid());
        return;
...
{
    DmsXcollie dmsXcollie("DMS:GetCurrentScreenPhyBounds", XCOLLIE_TIMEOUT_30S);
    if (foldScreenController_ != nullptr) {
