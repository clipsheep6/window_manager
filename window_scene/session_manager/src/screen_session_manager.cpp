...
void ScreenSessionManager::CreatePhysicalMirrorSessionInner(sptr<ScreenSession> session, ScreenId screenId,
    ScreenId defScreenId, ScreenProperty property)
{
    ...
    session->SetMirrorScreenType(MirrorScreenType::PHYSICAL_MIRROR);
...
sptr<ScreenSession> ScreenSessionManager::GetScreenSessionInner(ScreenId screenId, ScreenProperty property)
{
    ...
    if (phyMirrorEnable && screenId != SCREEN_ID_MAIN && screenId != SCREEN_ID_FULL && screenId != SCREEN_ID_PC_MAIN) {
        CreatePhysicalMirrorSessionInner(session, screenId, defScreenId, property);
    } else {
