...
bool ScreenSessionManager::SuspendBegin(PowerStateChangeReason reason)
{
    ...
    WLOGFI("[UL_POWER]SuspendBegin block screen power change is true, reason: %{public}u", static_cast<uint32_t>(reason));
...
{
    WLOGFI("[UL_POWER]state: %{publilc}u, reason: %{public}u", static_cast<uint32_t>(state), static_cast<uint32_t>(reason));
    ScreenPowerStatus status;
