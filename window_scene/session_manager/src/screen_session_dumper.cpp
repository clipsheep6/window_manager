...
void ScreenSessionDumper::OutputDumpInfo()
{
    ...
    UniqueFd ufd = UniqueFd(fd);
    int ret = dprintf(ufd.Get(), "%s\n", dumpInfo_.c_str());
    if (ret < 0) {
        WLOGFE("dprintf error. ret: %{public}", ret);
        return;
    }
    dumpInfo_.clear();
}
...
void ScreenSessionDumper::ExcuteDumpCmd()
{
    ...
    if (param_.empty() || param_[0] == ARG_DUMP_HELP) { //help command
        ShowHelpInfo();
    }
    ...
    if (param_[0] == ARG_DUMP_ALL) { // dump all info command
        ShowAllScreenInfo();
    }
    OutputDumpInfo();
}
...
void ScreenSessionDumper::DumpScreenInfoById(ScreenId id)
{
    ...
    oss << std::left << std::setw(LINE_WIDTH) << "VirtualWidth: "
        << screenInfo->VirtualWidth() << std::endl;
    oss << std::left << std::setw(LINE_WIDTH) << "VirtualHeight: "
        << screenInfo->VirtualHeight() << std::endl;
    ...
    if (modes.size() > modeId) {
        ...
        WLOGFW("modes size invalid. modes size: %{public}" PRIu64"", modes.size());
    }
    dumpInfo_.append(oss.str());
}
...
void ScreenSessionDumper::DumpScreenPropertyById(ScreenId id)
{
    ...
    oss << std::left << std::setw(LINE_WIDTH) << "ReqOrientation: "
        << static_cast<int32_t>(screenInfo->GetScreenRequestedOrientation()) << std::endl;
    ...
    oss << std::left << std::setw(LINE_WIDTH) << "AvailableArea<X,Y,W,H> "
        << screenInfo->GetAvailableArea().posX_ << ", "
        << screenInfo->GetAvailableArea().posY_ << ", "
        << screenInfo->GetAvailableArea().width_ << ", "
        << screenInfo->GetAvailableArea().height_ << ", " << std::endl;
    dumpInfo_.append(oss.str());
}
} // Rosen
} // OHOS