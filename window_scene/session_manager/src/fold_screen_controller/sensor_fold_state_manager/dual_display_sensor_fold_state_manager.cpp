...
namespace {
constexpr HiviewDFX::HiLogLabel LABEL = {LOGCORE, HILOG_DOMAIN_DISPLAY, "DualDisplaySensorFoldStateManager"};
...
void DualDisplaySensorFoldStateManager::RegisterApplicationStateObserver()
{
    ...
    if (flag != ERR_OK) {
        WLOGFE("Register app debug listener failed, flag = %{public}d", flag);
    } else {
...
void ApplicationStateObserver::OnForegroundApplicationChanged(const AppStateData &appStateData)
{
    if (appStateData.state == static_cast<int32_t>(ApplicationState::APP_STATE_FOREGROUND)) {
        ...
        WLOGFI("APP_STATE_FOREGROUND, pacjage= %{public}s", foregroundBundleName_.c_str());
    }
