...
WMError SessionManager::IsImmersiveFullScreenExisted(bool& immersive)
{
    WLOGFD("IsImmersiveFullScreenExisted");
    std::lock_guard<std::recursive_mutex> lock(mutex_);
    GetSceneSessionManagerProxy();
    if (!sceneSessionManagerProxy_) {
        WLOGFE("sceneSessionManagerProxy_ is nullptr");
    ...
    return static_cast<WMError>(sceneSessionManagerProxy_->IsImmersiveFullScreenExisted(immersive));
}
} // namespace OHOS::Rosen
