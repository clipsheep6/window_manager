...
void GravitySensorSubscriber::HandleGravitySensorEventCallback(SensorEvent *event)
{
    ...
    DeviceRotation sensorRotationConverted = ScreenRotationProperty::ConvertSensorToDeviceRotation(
        CalcSensorRotation(sensorDegree));
    ScreenRotationProperty::HandleSensorEventInput(sensorRotationConverted);
}
