...
int32_t ScreenSessionPublish::PublishEvents(const EventFwk::CommonEventData &eventData, std::string bundleName)
{
    if (publishInfo_ == nullptr) {
...
void ScreenSessionPublish::PublishCastPluggedEvent(const bool &isEnable)
{
    ...
    eventData.SetWant(*cesWantMap_[COMMON_EVENT_CAST_PLUGGED_CHANGED]);
...
void ScreenSessionPublish::PublishDisplayRotationEvent(const ScreenId &screenId, const Rotation &displayRotation)
{
    TLOGI(WmsLogTag::DMS, "start to publish display rotation event, screenId: %{public}d, displayRotation: %{public}d",
    ...
    TLOGE(WmsLogTag::DMS, "end of publish display rotation event");
}
} // namespace OHOS::Rosen