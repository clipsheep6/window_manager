...
void SceneInputManager::NotifyWindowIncoChangeFormSession(const sptr<SceneSession>& sceneSesion)
{
    if(sceneSessionDirty_){
...
void SceneInputManager::NotifyChangeInfo(){
    ...
    for(auto& iter : screenId2Windows){
        ...
        int32_t focusId = Rosen::SceneSessionManager::GetInstance().GetFocusedSession();
        MMI::InputManager::GetInstance()->UpdateWindowInfo(displayId, windowInfos, focusId);
