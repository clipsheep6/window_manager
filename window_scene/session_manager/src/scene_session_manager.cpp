    ...
    const sptr<SceneSession>& sceneSession)
{
    ...
    WLOGFD("wid = %{public}d, inWid = %{public}d, uiNId = %{public}d, bundleName = %{public}s",
    ...
    std::vector<sptr<AccessibilityWindowInfo>>& accessibilityInfo)
{
    for (const auto& sceneSession : sceneSessionList) {
        if (!FillWindowInfo(accessibilityInfo, sceneSession)) {
            TLOGW(WmsLogTag::WMS_EVENT, "fill accessibilityInfo failed");
        }
    }
}
...
void SceneSessionManager::NotifyAllAccessibilityInfo()
{
    ...
    for (const auto& item : accessibilityInfo) {
        WLOGFD("notify accessibilityWindow wid = %{public}d, inWid = %{public}d, bundle=%{public}s, \
            bounds=(x = %{public}d, y = %{public}d, w = %{public}d, h = %{public}d)",
