...
void SceneSessionManager::HandleCastScreenDisConnection(const sptr<SceneSession> sceneSession)
{
    ...
    auto screenSession = ScreenSessionManagerClient::GetInstance().GetScreenSessionById(sessionInfo.screenId_);
    if (screenSession == nullptr || screenSession->GetName() != "CastEngine") {
        return;
    }
