...
{
    TLOGD(WmsLogTag::DEFAULT, "GetUntouchableUnreliableWindowInfo Called.");
    if (!SessionPermission::IsSystemServiceCalling()) {
        TLOGE(WmsLogTag::DEFAULT, "GetUntouchableUnreliableWindowInfo only support for system service.");
        return WMError::WM_ERROR_NOT_SYSTEM_APP;
    }
    auto task = [this, windowId, &infos]() {
        ...
        for (const auto& [curWindowId, sceneSession] : sceneSessionMap_) {
            ...
            if (curWindowId == windowId) {
                ...
                FillUntouchableUnreliableWindowInfo(infos, sceneSession);
                continue;
            }
            if (sceneSession->GetTouchable()) {
                TLOGD(WmsLogTag::DEFAULT, "persistentId: %{public}d is touchable", curWindowId);
                continue;
            }
            ...
            if (CheckUnreliableWindowType(sceneSession->GetWindowType())) {
                TLOGI(WmsLogTag::DEFAULT, "GetUntouchableUnreliableWindowInfo, persistentId = %{public}d, "
