...
sptr<AAFwk::SessionInfo> SceneSessionManager::SetAbilitySessionInfo(const sptr<SceneSession>& scnSession)
{
    ...
    iTempToken_ = std::chrono::time_point_cast<std::chrono::milliseconds>(
    ...
    abilitySessionInfo->identityToken = iTempToken_;
...
{
    return AAFwk::AbilityManagerClient::GetInstance()->StartUIAbilityBySCB(abilitySessionInfo, false);
}
    ...
    sptr<SceneSession>& scnSession, bool isNewActive, const std::shared_ptr<std::promise<int32_t>>& promise)
{
    ...
    if (systemConfig_.backgroundswitch == false) {
        ...
        errCode = AAFwk::AbilityManagerClient::GetInstance()->StartUIAbilityBySCB(scnSessionInfo, false);
    } else {
    ...
    if (isColdStart) {
        scnSession.SetIdentityToken(iTempToken_);
