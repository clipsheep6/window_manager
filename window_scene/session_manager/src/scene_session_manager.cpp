    ...
    const std::vector<std::string>& params, std::string& dumpInfo)
{
    ...
    if (!session->GetSessionInfo().isSystem_) {
        ...
        dumpSessionPid_ = session->GetCallingPid();
...
{
    if (dumpSessionPid_ == INVALID_SESSION_ID) {
        WLOGFD("NotifyDumpInfoResult failed,session pid invalid");
        return;
    }
    pid_t callingPid = IPCSkeleton::GetCallingRealPid();
    if (dumpSessionPid_ != callingPid) {
        WLOGFE("Invalid calling");
        return;
    }
