...
void PrintLogGetIncrementWindoInfoList(const std::map<uint64_t, std::vector<MMI::WindowInfo>>& screen2windowInfo){
...
static MMI::WindowInfo PrepareWindowInfo(sptr<SceneSession> sceneSession, int action){
    ...
    const std::vector<int32_t> pointerChangeAreas{16, 5, 16, 5, 16, 5};
   ...
   MMI::Rect rect{windowRect.posX_, windowRect.posY_, windowRect.width_, windowRect.height_};
...
void SceneSessionDirty::NotifyWindowInfoChange(const sptr<SceneSession>& sceneSession, const WindowUpdateType& type){
    ...
    if(action != WindowAction::WINDOW_DELETE){
        ...
        const auto& e = windowinfo;
        WLOGFI("testywt : NotifyWindowInfoChange2 GetWindowInfoList windowInfoList id = %{public}d area.x = %{public}d area.y = %{public}d area.w = %{public}d  area.h = %{public}d agentWindowId = %{publid}d flags = %{public}d",
        e.id,  e.area.x, e.area.y, e.area.width, e.area.height, e.agentWindowId, e.flags);     
    }
    else
    {
    ...
    PushWindowInfoList(displayid, windowinfo);
