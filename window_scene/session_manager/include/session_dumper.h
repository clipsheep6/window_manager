...
#ifndef OHOS_ROSEN_SESSION_DUMPER_H
#define OHOS_ROSEN_SESSION_DUMPER_H
...
namespace OHOS {
namespace Rosen {
class SessionDumper : public RefBase {
public:
    ...
    explicit SessionDumper(const sptr<WindowRoot>& root) : windowRoot_(root) {}
...
private:
    ...
    WMError DumpScreenGroupWindowInfo(ScreenId screenGroupId, const sptr<WindowNodeContainer>& windowNodeContainer, std::string& dumpInfo);
    //bool IsValidDigitString(const std::string& windowIdStr);
    WMError DumpSpecifiedWindowInfo(uint32_t windowId, const std::vector<std::string>& params, std::string& dumpInfo);
...
#endif // OHOS_ROSEN_SESSION_DUMPER_H