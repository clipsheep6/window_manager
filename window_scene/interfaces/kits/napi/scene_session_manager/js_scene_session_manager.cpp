...
void JsSceneSessionManager::BindNativeFunctions(napi_env env, napi_value exportObj)
{
    ...
    BindModuleFunction(env, exportObj, "isSceneSessionValid", JsSceneSessionManager::IsSceneSessionValid);
    ...
    BindModuleFunction(env, exportObj, "checkSceneZOrder", JsSceneSessionManager::CheckSceneZOrder);
    ...
    BindModuleFunction(env, exportObj, "updateConfig", JsSceneSessionManager::UpdateConfig);
    ...
    BindModuleFunction(env, exportObj, "getIsLayoutFullScreen", JsSceneSessionManager::GetIsLayoutFullScreen);
    BindModuleFunction(env, exportObj, "switchFreeMultiWindow", JsSceneSessionManager::SwitchFreeMultiWindow);
    BindModuleFunction(env, exportObj, "getFreeMultiWindowConfig", JsSceneSessionManager::GetFreeMultiWindowConfig);
}
...
napi_value JsSceneSessionManager::OnRequestSceneSessionBackground(napi_env env, napi_callback_info info)
{
    ...
    if (errCode == WSErrorCode::WS_OK) {
        errCode = GetSceneSession(env, argv[0], sceneSession);
    }
    if (sceneSession == nullptr) {
        TLOGE(WmsLogTag::WMS_LIFE, "[NAPI]sceneSession is nullptr");
        napi_throw(env, CreateJsError(env, static_cast<int32_t>(WSErrorCode::WS_ERROR_SYSTEM_ABNORMALLY),
...
WSErrorCode JsSceneSessionManager::GetSceneSession(
    napi_env env, napi_value jsSceneSessionObj, sptr<SceneSession>& sceneSession)
{
    if (jsSceneSessionObj == nullptr) {
        TLOGE(WmsLogTag::WMS_LIFE, "[NAPI]Failed to get js scene session object");
        return WSErrorCode::WS_ERROR_INVALID_PARAM;
    } else {
        void* pointerResult = nullptr;
        napi_unwrap(env, jsSceneSessionObj, &pointerResult);
        auto jsSceneSession = static_cast<JsSceneSession*>(pointerResult);
