    ...
    const sptr<SceneSession>& panelSession)
{
    if (keyboardSession == nullptr || panelSession == nullptr) {
        TLOGI(WmsLogTag::WMS_LIFE, "[NAPI]keyboard or panel session is nullptr");
        return;
    }
    auto iter = jsCbMap_.find(CREATE_KEYBOARD_SESSION_CB);
    if (iter == jsCbMap_.end()) {
        TLOGE(WmsLogTag::WMS_LIFE, "[NAPI]Can't find callback, id: %{public}d", keyboardSession->GetPersistentId());
        return;
    }

    TLOGD(WmsLogTag::WMS_LIFE, "[NAPI]OnCreateKeyboardSession, id: %{public}d", keyboardSession->GetPersistentId());
    auto jsCallBack = iter->second;
...
void JsSceneSessionManager::ProcessCreateKeyboardSessionRegister()
{
    NotifyCreateKeyboardSessionFunc func = [this](const sptr<SceneSession>& keyboardSession,
        const sptr<SceneSession>& panelSession) {
        TLOGI(WmsLogTag::WMS_SYSTEM, "NotifyCreateKeyboardSessionFunc");
