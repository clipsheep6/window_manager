...
napi_value JsRootSceneSession::OnLoadContent(napi_env env, napi_callback_info info)
{
    ...
    napi_value storage = argc < ARGC_THREE ? nullptr : argv[2];
    ...
    napi_create_object(env, &context);
    if (context == nullptr) {
    ...
    auto contentStorage = std::shared_ptr<NativeReference>();
    if (storage == nullptr) {
        contentStorage = nullptr;
    } else {
...
void JsRootSceneSession::PendingSessionActivation(SessionInfo& info)
{
    ...
    auto complete = std::make_unique<NapiAsyncTask::CompleteCallback>(
        [info, jsCallBack](napi_env env, NapiAsyncTask& task, int32_t status) {
            ...
            napi_call_function(env, NapiGetUndefined(env), jsCallBack->GetNapiValue(), ArraySize(argv), argv, &callResult);
        });
