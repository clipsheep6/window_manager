...
void JsRootSceneSession::PendingSessionActivationInner(std::shared_ptr<SessionInfo> sessionInfo)
{
    ...
    auto task = [sessionInfo, jsCallBack, env_ref]() {
        ...
        TLOGI(WmsLogTag::WMS_LIFE, "[NAPI]PendingSessionActivationInner task success, id %{public}d", sessionInfo->persistentId_);
...
void JsRootSceneSession::PendingSessionActivation(SessionInfo& info)
{
    ...
    if (info.want != nullptr) {
        ...
        TLOGI(WmsLogTag::WMS_LIFE, "[NAPI]session: %{public}d isNeedBackToOther: %{public}d", sceneSession->GetPersistentID(), isNeedBackToOther);
