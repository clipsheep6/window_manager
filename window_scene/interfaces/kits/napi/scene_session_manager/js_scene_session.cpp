    ...
    : engine_(engine), session_(session)
{
    listenerFunc_ = {
        ...
        { SESSION_STATE_CHANGE_CB,        &JsSceneSession::ProcessSessionStageChangeRegister },
    };
}
...
void JsSceneSession::ProcessSessionStageChangeRegister()
{
...
bool JsSceneSession::IsCallbackRegistered(const std::string& type, NativeValue* jsListenerObject)
{
    if (jsCbMap_.empty() || jsCbMap_.find(type) == jsCbMap_.end()) {
        WLOGFD("[NAPI]Method %{public}s has not been registered", type.c_str());
        return false;
    }
