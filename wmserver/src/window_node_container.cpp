...
WMError WindowNodeContainer::AddWindowNode(sptr<WindowNode>& node, sptr<WindowNode>& parentNode)
{
    if (!node->startingWindowShown_) {
        ...
        for (auto& displayId : node->GetShowingDisplays()) {
            AddNodeOnRSTree(node, displayId, WindowUpdateType::WINDOW_UPDATE_ADDED, displayId, node->isPlayAnimationShow_);
        }
    } else {
                                                                ...
                                                                const std::vector<DisplayId>& curShowingDisplays)
{
    ...
    for (auto& displayId : lastShowingDisplays) {
        if (std::find(curShowingDisplays.begin(), curShowingDisplays.end(), displayId) == curShowingDisplays.end()) {
            RemoveNodeFromRSTree(node, displayId, WindowUpdateType::WINDOW_UPDATE_ACTIVE, *(curShowingDisplays.begin()));
    ...
    for (auto& displayId : curShowingDisplays) {
        if (std::find(lastShowingDisplays.begin(), lastShowingDisplays.end(), displayId) == lastShowingDisplays.end()) {
            AddNodeOnRSTree(node, displayId, WindowUpdateType::WINDOW_UPDATE_ACTIVE, displayId);
...
WMError WindowNodeContainer::RemoveWindowNode(sptr<WindowNode>& node)
{
    ...
    for (auto& displayId : node->GetShowingDisplays()) {
        RemoveNodeFromRSTree(node, displayId, WindowUpdateType::WINDOW_UPDATE_REMOVED, displayId, node->isPlayAnimationHide_);
    }
...
bool WindowNodeContainer::AddNodeOnRSTree(sptr<WindowNode>& node, DisplayId displayId, WindowUpdateType type,
...
bool WindowNodeContainer::RemoveNodeFromRSTree(sptr<WindowNode>& node, DisplayId displayId, WindowUpdateType type,
    DisplayId parentDisplayId, bool animationPlayed)
{
    WM_FUNCTION_TRACE();
    if (node->GetWindowType() == WindowType::WINDOW_TYPE_APP_COMPONENT) {
