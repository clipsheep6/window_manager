...
void WindowRoot::ProcessExpandDisplayCreate(DisplayId displayId, ScreenId screenGroupId)
{
    const sptr<DisplayInfo> displayInfo = DisplayManagerServiceInner::GetInstance().GetDisplayById(displayId);
        ...
        auto displayInfoMap = GetDisplayRectsForAllDisplay(displayIdMap_[screenGroupId]);
...
std::map<DisplayId, sptr<DisplayInfo>> WindowRoot::GetDisplayRectsForAllDisplay(
    const std::vector<DisplayId>& displayIdVec)
{
...
void WindowRoot::ProcessDisplayDestroy(DisplayId displayId)
{
    ...
    WLOGFE("[Display Destroy] displayId: %{public}" PRIu64"", displayId);

    std::vector<uint32_t> destroyIds;
    auto displayInfoMap = GetDisplayRectsForAllDisplay(displayIdVec);
    container->GetMutiDisplayController()->ProcessDisplayDestroy(displayId, displayInfoMap, destroyIds);
...
void WindowRoot::ProcessDisplayChange(const sptr<DisplayInfo>& displayInfo, DisplayStateChangeType type)
{
    ...
    container->GetMutiDisplayController()->ProcessDisplayChange(displayInfo, type);
}
...
Rect WindowRoot::GetFullDisplayRect(DisplayId displayId) const
{
    ...
    return container->GetFullDisplayRect();
}
