...
WMError WindowRoot::AddWindowNode(uint32_t parentId, sptr<WindowNode>& node)
{
    ...
    int windowNumber = container->GetWindowNumber(WindowType::WINDOW_TYPE_APP_MAIN_WINDOW);
    if(windowNumber >= maxWindowNumber_)
    {
        container->MinimizeFirstAppWindow();
    }
...
void WindowRoot::SetMaxWindowNumber(int windowNum)
{
    maxWindowNumber_ = windowNum;
}
