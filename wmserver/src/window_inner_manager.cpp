...
void WindowInnerManager::DrawSurface(const sptr<Window>& window, uint32_t color)
{
    ...
    if (!framePtr) {
        WLOGFI("DrawSurface frameptr is nullptr");
        return;
    }
    auto canvas = framePtr->GetCanvas();
    if (!canvas) {
        WLOGFI("DrawSurface canvas is nullptr");
        return;
    }
    SkPaint paint;
    paint.setAntiAlias(true);
    paint.setStyle(SkPaint::kFill_Style);
    paint.setStrokeWidth(20); // Stroke width : 20
...
sptr<Window> WindowInnerManager::CreateWindow(uint32_t displayId, const WindowType& type, const Rect& rect)
{
    ...
    if (window == nullptr) {
        ...
        dividerMap_.insert({ displayId, window });
...
void WindowInnerManager::SendMessage(InnerWMCmd cmdType, uint32_t displayId, Rect dividerRect)
{
    std::unique_lock<std::mutex> lk(mutex_);
...
void WindowInnerManager::Init()
{
    ...
    WLOGFI("Init inner window manager success!");
}
}
}