...
WMError WindowManagerService::GetUntouchableUnreliableWindowInfo(int32_t windowId,
    std::vector<sptr<UntouchableUnreliableWindowInfo>>& infos)
{
    if (!Permission::IsSystemServiceCalling()) {
        WLOGFE("get untouchable unreliable window info permission denied!");
        return WMError::WM_ERROR_NOT_SYSTEM_APP;
    }
    auto task = [this, windowId, &infos]() {
        return windowController_->GetUntouchableUnreliableWindowInfo(windowId, infos);
    };
    return PostSyncTask(task, "GetUntouchableUnreliableWindowInfo");
}
