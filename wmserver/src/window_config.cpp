...
{
    const auto& config = WindowManagerConfig::GetIntNumbersConfig();
     if (config.count(name) != 0) {
        auto numbers = config.at(name);
        if (numbers.size() == 1) {
            if (numbers[0] > 0) {
...
void WindowConfig::initDefaultFloatingWindow()
{
    ...
    if (config.count("defaultFloatingWindow") != 0) {
        std::vector<int>& numbers = config.at("defaultFloatingWindow");
