...
namespace {
    ...
    constexpr uint32_t MAX_WIN_NUM_VERTICAL = 3;
...
void WindowLayoutPolicyTile::InitTileWindowRects()
{
    ...
    x = edgeInterval;
    w = (limitRect_.width_ - edgeInterval * 2 - midInterval) / 2;
    // calc doubleRect
    ...
    // calc tripleRect
    w = (limitRect_.width_ - edgeInterval * 2 - midInterval * 2) / 3;
    tripleRects_[0] = {x, y, w, h};
    tripleRects_[1] = {x + w + midInterval, y, w, h};
    tripleRects_[2] = {x + w * 2 + midInterval * 2, y, w, h};
...
{
    WM_FUNCTION_TRACE();
    WLOGFI("AddWindowNode");
    if (WindowHelper::IsMainWindow(node->GetWindowType())) {
        WLOGFI("AddWindowNode1");
        UpdateForegroundNodeQueue(node);
        LayoutForegroundNodeQueue();
    } else {
        WLOGFI("AddWindowNode2");
...
void WindowLayoutPolicyTile::AssignNodePropertyForTileWindows()
{
    ...
    if (num == 1) {
        ...
        for (int i = 0; i < foregroundNodes_.size(); i++) {
