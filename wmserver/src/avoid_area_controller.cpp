...
{
    AvoidNodesMapPtr avoidNodesMapPtr = std::make_unique<std::map<uint32_t, sptr<WindowNode>>>();
...
AvoidNodesMapPtr AvoidAreaController::GetAvoidNodesByDisplayId(DisplayId displayId)
{
    if ((const_cast<AvoidAreaController*>(this)->avoidNodesMaps_).find(displayId) !=
        ...
        return std::move((const_cast<AvoidAreaController*>(this)->avoidNodesMaps_)[displayId]);
    }
    return nullptr;
}
