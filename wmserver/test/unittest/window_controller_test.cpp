...
/**
 * @tc.name: UpdateFocusIfNeededWhenRaiseWindow
 * @tc.desc: check UpdateFocusIfNeededWhenRaiseWindow
 * @tc.type: FUNC
 */
HWTEST_F(WindowControllerTest, RaiseToAppTop, Function | SmallTest | Level3)
{
    ...
    sptr<WindowNode> parent = new (std::nothrow)WindowNode();
    parent->property_->windowId_ = 99;
    parent->currentVisibility_ = true;
    windowRoot_->windowNodeMap_.insert(std::make_pair(parent->GetWindowId(), parent));

    sptr<WindowNode> node = new (std::nothrow)WindowNode();
    node->property_->windowId_ = 100;
    node->property_->parentId_ = 99;
    node->property_->SetFocusable(false);
    node->property_->displayId_ = 1;
    node->currentVisibility_ = true;
    node->parent_ = parent;
    windowRoot_->windowNodeMap_.insert(std::make_pair(node->GetWindowId(), node));

    windowController_->UpdateFocusIfNeededWhenRaiseWindow(node);
    ASSERT_NE(container->focusedWindow_, node->GetWindowId());
    
    node->property_->SetFocusable(true);
    ASSERT_NE(container->focusedWindow_, node->GetWindowId());

    container->focusedWindow_ = 98;
    windowController_->UpdateFocusIfNeededWhenRaiseWindow(node);
    ASSERT_NE(container->focusedWindow_, node->GetWindowId());

    container->focusedWindow_ = 99;
    windowController_->UpdateFocusIfNeededWhenRaiseWindow(node);
    ASSERT_EQ(container->focusedWindow_, node->GetWindowId());

    container->focusedWindow_ = 100;
    windowController_->UpdateFocusIfNeededWhenRaiseWindow(node);
    ASSERT_EQ(container->focusedWindow_, node->GetWindowId());
