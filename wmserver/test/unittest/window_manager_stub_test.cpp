 ...
 * @tc.desc: test TRANS_ID_GET_UNTOUCHABLE_UNRELIABLE_WINDOW_INFO_ID success
 * @tc.type: FUNC
 */
HWTEST_F(WindowManagerStubTest, OnRemoteRequest23, Function | SmallTest | Level2)
{
    ...
    uint32_t code = static_cast<uint32_t>(
        IWindowManager::WindowManagerMessage::TRANS_ID_GET_UNTOUCHABLE_UNRELIABLE_WINDOW_INFO_ID);
    int res = stub_->OnRemoteRequest(code, data, reply, option);
    EXPECT_EQ(res, 0);
}
}
