 ...
 * @tc.desc: ConfigStartingWindowAnimation02 test
 * @tc.type: FUNC
 */
HWTEST_F(WindowManagerServiceTest, ConfigStartingWindowAnimation02, Function | SmallTest | Level2)
{
    const auto& config = WindowManagerConfig::GetConfig();
    WindowManagerConfig::ConfigItem item = config["decor"];
    if (item.IsMap()) {
        wms->ConfigDecor(item);
    }
    item = config["startWindowTransitionAnimation"];
    item.type_ = XmlConfigBase::ValueType::BOOL;
    ASSERT_TRUE(wms != nullptr);
    wms->ConfigStartingWindowAnimation(item);
    item.type_ = XmlConfigBase::ValueType::MAP;
    ASSERT_TRUE(wms != nullptr);
    wms->ConfigStartingWindowAnimation(item);
    item.type_ = XmlConfigBase::ValueType::INTS;
    ASSERT_TRUE(wms != nullptr);
    wms->ConfigStartingWindowAnimation(item);
    item.type_ = XmlConfigBase::ValueType::FLOATS;
    ASSERT_TRUE(wms != nullptr);
    wms->ConfigStartingWindowAnimation(item);
}
...
HWTEST_F(WindowManagerServiceTest, CreateWindow02, Function | SmallTest | Level2)
{
    ...
    sptr<WindowProperty> property = new WindowProperty();
    property->SetWindowType(WindowType::WINDOW_TYPE_INPUT_METHOD_FLOAT);
    ASSERT_EQ(WMError::WM_ERROR_NULLPTR, wms->CreateWindow(iWindow, property, surfaceNode, id, nullptr));
    wms->DestroyWindow(id, true);
