...
{
    pImpl_->SetVirtualScreenSecurityExemption(screenId, pid, windowIdList);
}

DMError DisplayManager::Impl::SetVirtualScreenSecurityExemption(ScreenId screenId, uint32_t pid,
    std::vector<uint64_t>& windowIdList)
{
    return SingletonContainer::Get<DisplayManagerAdapter>().SetVirtualScreenSecurityExemption(screenId, pid, windowIdList);
}
} // namespace OHOS::Rosen
