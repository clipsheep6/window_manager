...
=======
    ...
    if (pImpl_->powerEventListeners_.empty() && pImpl_->powerEventListenerAgent_ != nullptr) {
        SingletonContainer::Get<DisplayManagerAdapter>().UnregisterDisplayManagerAgent(pImpl_->powerEventListenerAgent_,
>>>>>>> Stashed changes
            DisplayManagerAgentType::DISPLAY_POWER_EVENT_LISTENER);
...
=======
    std::lock_guard<std::mutex> lock(pImpl_->mutex_);
    if (pImpl_->displayStateCallback_ != nullptr) {
    ...
    if (pImpl_->displayStateAgent_ == nullptr) {
        ...
        SingletonContainer::Get<DisplayManagerAdapter>().RegisterDisplayManagerAgent(pImpl_->displayStateAgent_,
>>>>>>> Stashed changes
            DisplayManagerAgentType::DISPLAY_STATE_LISTENER);
    }
