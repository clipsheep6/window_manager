...
public:
    WindowDisplayChangeAdapter(const sptr<IRemoteObject>& token, const sptr<IDisplayInfoChangedListenter>& listener);
    ~WindowDisplayChangeAdapter();

public:
    void OnCreate(DisplayId displayId) override;
    void OnDestroy(DisplayId displayId) override;
    void OnChanged(DisplayId displayId) override;

    void OnDisplayInfoChanged(const sptr<IRemoteObject>& token,
...
private:
    ...
    const sptr<IDisplayInfoChangedListenter> displayInfoChangeListener_ {nullptr};
