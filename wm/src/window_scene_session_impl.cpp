...
WMError WindowSceneSessionImpl::MoveToAsync(int32_t x, int32_t y)
{
    ...
    CHECK_HOST_SESSION_RETURN_ERROR_IF_NULL(hostSession, WMError::WM_ERROR_INVALID_WINDOW);
    auto ret = hostSession->UpdateSessionRect(wsRect, SizeChangeReason::MOVE);
    if (state_ == WindowState::STATE_SHOWN && property_ && property_->GetLayoutCallback() != nullptr) {
        property_->GetLayoutCallback()->ResetLock();
        property_->GetLayoutCallback()->GetResult(WINDOW_LAYOUT_TIMEOUT);
    }
    return static_cast<WMError>(ret);
}
