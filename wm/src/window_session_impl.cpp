...
void WindowSessionImpl::RegisterSessionRecoverListener(SessionManager::SessionRecoverCallbackFunc callbackFunc)
{
...
WMError WindowSessionImpl::RecoverAndReconnectSceneSession()
{
    ...
    if (property_ && context_ && context_->GetHapModuleInfo() && abilityContext && abilityContext->GetAbilityInfo()) {
        ...
        info.persistentId_ = property_->GetPersistentId();
    } else {
