...
void JsPipController::PiPActionObserverImpl::OnActionEvent(const std::string& actionEvent, int32_t statusNum)
{
    std::lock_guard<std::mutex> lock(mtx_);
    WLOGI("OnActionEvent is called, actionEvent: %{public}s", actionEvent.c_str());
    auto jsCallback = jsCallBack_;
    std::unique_ptr<NapiAsyncTask::CompleteCallback> complete = std::make_unique<NapiAsyncTask::CompleteCallback> (
        [jsCallback, actionEvent, statusNum] (napi_env env, NapiAsyncTask &task, int32_t status) {
            napi_value argv[2] = {CreateJsValue(env, actionEvent), CreateJsValue(env, statusNum)};
            CallJsMethod(env, jsCallback->GetNapiValue(), argv, ArraySize(argv));
        }
    );
