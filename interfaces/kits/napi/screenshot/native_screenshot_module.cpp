...
static void AsyncGetScreenshot(napi_env env, std::unique_ptr<Param> &param)
{
    if (!CheckCallingPermission("ohos.permission.CAPTURE_SCREEN")) {
        ...
        param->errMessage = "Do not have permission: ohos.permission.CAPTURE_SCREEN";
        return;
    }
    ...
    if (param->image == nullptr) {
        ...
        param->errMessage = "Get Screenshot failed!";
        return;
    }
...
napi_value MainFunc(napi_env env, napi_callback_info info)
{
    ...
    if (argc == 1 && GetType(env, argv[0]) == napi_object) {
    ...
    } else if (argc == 2 && GetType(env, argv[0]) == napi_object &&
        GetType(env, argv[1]) == napi_function) { // the number of input parameters is 2
