...
{
    WLOGFD("OnCreate");
    napi_value nativeString = nullptr;
    napi_value nativeContext = nullptr;
    napi_value nativeType = nullptr;
    ...
    if (argc >= 2 && GetType(env, argv[0]) == napi_string) { // 2: minimum params num
        ...
        callback = (argc == 2) ? nullptr :
            (GetType(env, argv[2]) == napi_function ? argv[2] : nullptr); // 2: index of callback
    } else if (argc >= 3) { // 3: minimum params num
        nativeContext = GetType(env, argv[3]) == napi_object ? argv[0] : nullptr;
        nativeString = argv[1];
        nativeType = argv[2]; // 2: index of type
        // 3: minimum params num;
        callback = (argc == 3) ? nullptr :
            (GetType(env, argv[3]) == napi_function ? argv[3] : nullptr); // 3: index of callback
    }
...
napi_value JsWindowManager::OnSetWaterMarkImage(napi_env env, napi_callback_info info)
{
    ...
    pixelMap = OHOS::Media::PixelMapNapi::GetPixelMap(reinterpret_cast<napi_env>(env),
    ...
    napi_value nativeBool = nativeBoolean;
    if (nativeBool == nullptr) {
        WLOGFE("Failed to convert parameter to bool");
        napi_throw(env, CreateJsError(env, static_cast<int32_t>(WmErrorCode::WM_ERROR_INVALID_PARAM)));
        return NapiGetUndefined(env);
    ...
    bool isShow = false;
    napi_get_value_bool(env, nativeBool, &isShow);
