    ...
    napi_env env, NapiAsyncTask& task)
{
    ...
    if (contextPtr == nullptr || context == nullptr) {
        task.Reject(env, JsErrorUtils::CreateJsError(env, WmErrorCode::WM_ERROR_CONTEXT_ABNORMALLY,
            "Context is nullptr"));
    ...
    if (windowOption->GetWindowType() == WindowType::WINDOW_TYPE_FLOAT ||
        ...
        if (abilityContext != nullptr) {
            if (!CheckCallingPermission("ohos.permission.SYSTEM_FLOAT_WINDOW")) {
                task.Reject(env, JsErrorUtils::CreateJsError(env, WmErrorCode::WM_ERROR_NO_PERMISSION,
    ...
    if (window != nullptr && wmErrorCode == WmErrorCode::WM_OK) {
        ...
        task.Reject(env, JsErrorUtils::CreateJsError(env, wmErrorCode, "Create window failed"));
    }
}
    ...
    napi_env env, NapiAsyncTask& task)
{
    ...
    auto context = static_cast<std::weak_ptr<AbilityRuntime::Context>*>(contextPtr);
    if (contextPtr == nullptr || context == nullptr) {
        task.Reject(env, JsErrorUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR, "Context is nullptr"));
        WLOGFE("Context is nullptr");
        return;
    }
    if (winType == WindowType::WINDOW_TYPE_FLOAT || winType == WindowType::WINDOW_TYPE_FLOAT_CAMERA) {
        ...
        if (abilityContext != nullptr) {
            if (!CheckCallingPermission("ohos.permission.SYSTEM_FLOAT_WINDOW")) {
                task.Reject(env, JsErrorUtils::CreateJsError(env, WMError::WM_ERROR_INVALID_PERMISSION,
                    "TYPE_FLOAT CheckCallingPermission failed"));
                return;
            }
        }
    ...
    if (windowOption == nullptr) {
        task.Reject(env, JsErrorUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR, "New window option failed"));
        WLOGFE("New window option failed");
        return;
    }
    ...
    napi_env env, NapiAsyncTask& task, bool newErrorCode = false)
{
    ...
    if (windowOption == nullptr) {
        task.Reject(env, JsErrorUtils::CreateJsError(env,
            newErrorCode ? WmErrorCode::WM_ERROR_SYSTEM_ABNORMALLY : WMError::WM_ERROR_NULLPTR,
                "New window option failed"));
        WLOGFE("New window option failed");
        return;
    }
    ...
    if (window != nullptr) {
        ...
        WLOGFE("Create window failed");
        task.Reject(env, JsErrorUtils::CreateJsError(env,
            newErrorCode ? WmErrorCode::WM_ERROR_STATE_ABNORMALLY : WMError::WM_ERROR_NULLPTR, "Create window failed"));
    }
}
...
napi_value JsWindowManager::OnCreateWindow(napi_env env, napi_callback_info info)
{
    ...
    napi_get_cb_info(env, info, &argc, argv, nullptr, nullptr);
    if (argc < 1) {
        napi_throw(env, JsErrorUtils::CreateJsError(env, WmErrorCode::WM_ERROR_INVALID_PARAM));
        ...
        [=](napi_env env, NapiAsyncTask& task, int32_t status) {
            ...
            if (jsWindowObj != nullptr && jsWindowObj->GetNapiValue() != nullptr) {
                ...
                if (window == nullptr) {
                    ...
                    task.Reject(env, JsErrorUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR, "Cannot find window"));
                } else {
                    task.Resolve(env, CreateJsWindowObject(env, window));
...
napi_value JsWindowManager::OnFindWindowSync(napi_env env, napi_callback_info info)
{
    ...
    if (jsWindowObj != nullptr && jsWindowObj->GetNapiValue() != nullptr) {
        ...
        sptr<Window> window = Window::Find(windowName);
        if (window == nullptr) {
            napi_throw(env, JsErrorUtils::CreateJsError(env, WmErrorCode::WM_ERROR_STATE_ABNORMALLY));
            return NapiGetUndefined(env);
        } else {
    ...
    NapiAsyncTask::CompleteCallback complete =
        [=](napi_env env, NapiAsyncTask& task, int32_t status) {
            ...
            if (ret == WmErrorCode::WM_OK) {
                ...
                task.Reject(env, JsErrorUtils::CreateJsError(env, ret, "OnMinimizeAll failed"));
            }
        };
    napi_value lastParam = (argc <= 1) ? nullptr :
    ...
    NapiAsyncTask::CompleteCallback complete =
        [=](napi_env env, NapiAsyncTask& task, int32_t status) {
            ...
            if (ret == WmErrorCode::WM_OK) {
                ...
                task.Reject(env, JsErrorUtils::CreateJsError(env, ret, "OnToggleShownStateForAllAppWindows failed"));
            }
        };
    size_t argc = 4;
    napi_value argv[4] = {nullptr};
...
static napi_value GetTopWindowTask(void* contextPtr, napi_env env, napi_value callback, bool newApi)
{
    ...
    NapiAsyncTask::CompleteCallback complete = [lists, newApi](napi_env env, NapiAsyncTask& task, int32_t status) {
        if (lists == nullptr) {
            task.Reject(env, JsErrorUtils::CreateJsError(env,
                newApi ? WmErrorCode::WM_ERROR_STATE_ABNORMALLY : WMError::WM_ERROR_NULLPTR, "napi abnormal"));
            return;
        }
        if (lists->errorCode != 0) {
            task.Reject(env, JsErrorUtils::CreateJsError(env,
                newApi ? static_cast<WmErrorCode>(lists->errorCode) : static_cast<wmError>(lists->errorCode),
                lists->errMsg));
        ...
        if (lists->window == nullptr) {
            task.Reject(env, JsErrorUtils::CreateJsError(env,
                newApi ? WmErrorCode::WM_ERROR_STATE_ABNORMALLY : WMError::WM_ERROR_NULLPTR, "Get top window failed"));
            return;
        }
    ...
    NapiAsyncTask::CompleteCallback complete =
        [=](napi_env env, NapiAsyncTask& task, int32_t status) {
            ...
            if (ret == WmErrorCode::WM_OK) {
                ...
                task.Reject(env, JsErrorUtils::CreateJsError(env, ret, "SetWindowLayoutMode failed"));
            }
        };
    ...
    NapiAsyncTask::CompleteCallback complete =
        [gestureNavigationEnable](napi_env env, NapiAsyncTask& task, int32_t status) {
            ...
            if (ret == WmErrorCode::WM_OK) {
                ...
                task.Reject(env, JsErrorUtils::CreateJsError(env, ret, "SetGestureNavigationEnabled failed"));
            }
        };
    // 1: maximum params num; 1: index of callback
    napi_value lastParam = (argc <= 1) ? nullptr :
...
napi_value JsWindowManager::OnSetWaterMarkImage(napi_env env, napi_callback_info info)
{
    ...
    if (!Permission::IsSystemCalling()) {
        ...
        napi_throw(env, JsErrorUtils::CreateJsError(env, WmErrorCode::WM_ERROR_NOT_SYSTEM_APP));
        return NapiGetUndefined(env);
    }
    ...
    NapiAsyncTask::CompleteCallback complete =
        [=](napi_env env, NapiAsyncTask& task, int32_t status) {
            ...
            if (ret == WmErrorCode::WM_OK) {
                ...
                task.Reject(env, JsErrorUtils::CreateJsError(env, ret, "ShiftAppWindowFocus failed"));
            }
        };
