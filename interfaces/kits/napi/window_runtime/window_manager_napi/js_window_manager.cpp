    ...
    napi_env env, NapiAsyncTask& task, bool newErrorCode = false)
{
    ...
    if (window != nullptr) {
        ...
        if (newErrorCode) {
            ...
            task.Reject(env, JsErrorUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR, "Create window failed"));
        }
    }
}
    ...
    NapiAsyncTask::CompleteCallback complete = [lists, newApi](napi_env env, NapiAsyncTask& task, int32_t status) {
        if (lists == nullptr) {
            if (newApi) {
                ...
                task.Reject(env, JsErrorUtils::CreateJsError(env, WMError::WM_ERROR_NULLPTR, "napi abnormal"));
            }
            return;
        }
        if (lists->errorCode != 0) {
            if (newApi) {
                task.Reject(env, JsErrorUtils::CreateJsError(env, static_cast<WmErrorCode>(lists->errorCode),
                    lists->errMsg));
            } else {
                task.Reject(env, JsErrorUtils::CreateJsError(env, static_cast<WMError>(lists->errorCode),
                    lists->errMsg));
            }
