...
void JsWindowListener::OnSystemBarPropertyChange(DisplayId displayId, const SystemBarRegionTints& tints)
{
    ...
    std::unique_ptr<NapiAsyncTask::CompleteCallback> complete = std::make_unique<NapiAsyncTask::CompleteCallback> (
        [self = weakRef_, displayId, tints, eng = env_] (napi_env env,
            ...
            napi_set_named_property(env, propertyValue, "displayId", CreateJsValue(eng, static_cast<uint32_t>(displayId)));
            napi_set_named_property(env, propertyValue, "regionTint", CreateJsSystemBarRegionTintArrayObject(eng, tints));
