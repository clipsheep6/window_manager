...
napi_value JsWindow::IsImmersiveFullScreen(napi_env env, napi_callback_info info)
{
    WLOGI("IsImmersiveFullScreen");
    JsWindow* me = CheckParamsAndGetThis<JsWindow>(env, info);
    return (me != nullptr) ? me->OnIsImmersiveFullScreen(env, info) : nullptr;
}
...
napi_value JsWindow::OnIsImmersiveFullScreen(napi_env env, napi_callback_info info)
{
    WMError errCode = WMError::WM_OK;
    size_t argc = 4;
    napi_value argv[4] = {nullptr};
    ...
    NapiAsyncTask::CompleteCallback complete =
        [weakToken, errCode](napi_env env, NapiAsyncTask& task, int32_t status) {
            ...
            if (errCode != WMError::WM_OK) {
                WLOGFE("window is nullptr or get invalid params");
                task.Reject(env, CreateJsError(env, static_cast<int32_t>(errCode)));
                return;
            }
    ...
    napi_value lastParam = (argc == 0) ? nullptr : (GetType(env, argv[0]) == napi_function ? argv[0] : nullptr);
    napi_value result = nullptr;
    NapiAsyncTask::Schedule("JsWindow::OnIsImmersiveFullScreen",
        env, CreateAsyncTaskWithLastParam(env, lastParam, nullptr, std::move(complete), &result));
    return result;
}
