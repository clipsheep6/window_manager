...
napi_value JsWindow::OnSetWindowType(napi_env env, napi_callback_info info)
{
    ...
    if (errCode == WMError::WM_OK) {
        ...
        if (nativeType == nullptr) {
            ...
            errCode = WMError::WM_ERROR_INVALID_PARAM;
        } else if (resultValue >= static_cast<uint32_t>(WindowType::SYSTEM_WINDOW_BASE) &&
            ...
            if (JS_TO_NATIVE_WINDOW_TYPE_MAP.count(
                static_cast<ApiWindowType>(resultValue)) != 0) {
                winType = JS_TO_NATIVE_WINDOW_TYPE_MAP.at(
                    static_cast<ApiWindowType>(resultValue));
            } else {
                WLOGFE("Do not surppot this type: %{public}u", resultValue);
                errCode = WMError::WM_ERROR_INVALID_PARAM;
            }
        }
    }
    ...
    sptr<Window> weakWindow, napi_env env, NapiAsyncTask& task, bool isLoadedByName)
{
    ...
    if (isLoadedByName) {
        ...
        ret = WM_JS_TO_ERROR_CODE_MAP.at(weakWindow->SetUIContent(contextUrl, env, nativeStorage, false, ability));
    }
    if (ret == WmErrorCode::WM_OK) {
        task.Resolve(env, NapiGetUndefined(env));
    } else {
...
napi_value JsWindow::OnSetPreferredOrientation(napi_env env, napi_callback_info info)
{
    ...
    if (argc < 1) { // 1: params num
        ...
        if (nativeType == nullptr) {
            ...
            uint32_t resultValue = 0;
            napi_get_value_uint32(env, nativeType, &resultValue);
            auto requestedApiOrientation = static_cast<ApiOrientation>(resultValue);
...
napi_value JsWindow::OnRaiseAboveTarget(napi_env env, napi_callback_info info)
{
    ...
    if (errCode == WmErrorCode::WM_OK) {
        ...
        if (nativeVal == nullptr || resultValue <= 0) {
...
napi_value JsWindow::OnSetForbidSplitMove(napi_env env, napi_callback_info info)
{
    ...
    if (errCode == WmErrorCode::WM_OK) {
        ...
        if (nativeVal == nullptr) {
            ...
            napi_get_value_bool(env, nativeVal, &isForbidSplitMove);
        }
    }
    if (errCode == WmErrorCode::WM_ERROR_INVALID_PARAM) {
        napi_throw(env, CreateJsError(env, static_cast<int32_t>(WmErrorCode::WM_ERROR_INVALID_PARAM)));
...
napi_value JsWindow::OnOpacity(napi_env env, napi_callback_info info)
{
    ...
    if (!WindowHelper::IsSystemWindow(windowToken_->GetType())) {
        ...
        napi_throw(env, CreateJsError(env, static_cast<int32_t>(WmErrorCode::WM_ERROR_INVALID_CALLING)));
        return NapiGetUndefined(env);
    }
    napi_value nativeVal = argv[0];
    if (nativeVal == nullptr) {
    ...
    double result = 0.0;
    napi_get_value_double(env, nativeVal, &result);
    if (MathHelper::LessNotEqual(result, 0.0) || MathHelper::GreatNotEqual(result, 1.0)) {
...
napi_value JsWindow::OnSetCornerRadius(napi_env env, napi_callback_info info)
{
    ...
    if (nativeVal == nullptr || MathHelper::LessNotEqual(radius, 0.0)) {
...
napi_value JsWindow::OnSetShadow(napi_env env, napi_callback_info info)
{
    ...
    { // parse the 1st param: radius
        napi_value nativeVal = argv[0];
        napi_get_value_double(env, nativeVal, &result);
        if (nativeVal == nullptr || MathHelper::LessNotEqual(result, 0.0)) {
            napi_throw(env, CreateJsError(env, static_cast<int32_t>(WmErrorCode::WM_ERROR_INVALID_PARAM)));
            return NapiGetUndefined(env);
        }
    ...
    if ((ret == WmErrorCode::WM_OK) && argc >= 3) { // parse the 3rd param: offsetX
        napi_value nativeVal = argv[2]; // 2: the 3rd param
        if (nativeVal != nullptr) {
            napi_get_value_double(env, nativeVal, &result);
    ...
    if ((ret == WmErrorCode::WM_OK) && argc >= 4) { // parse the 4th param: offsetY
        napi_value nativeVal = argv[3]; // 3: the 4th param
        if (nativeVal != nullptr) {
            napi_get_value_double(env, nativeVal, &result);
...
napi_value JsWindow::OnSetBackdropBlurStyle(napi_env env, napi_callback_info info)
{
    ...
    napi_get_value_uint32(env, nativeMode, &resultValue);
    if (nativeMode == nullptr ||
        resultValue > static_cast<uint32_t>(WindowBlurStyle::WINDOW_BLUR_THICK)) {
