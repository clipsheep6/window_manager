...
static int ctorCnt1 = 0;
static int dtorCnt1 = 0;
static int ctorCnt2 = 0;
static int dtorCnt2 = 0;

JsTransitionContext::JsTransitionContext(sptr<Window> window, bool shownState)
    : windowToken_(window), shownState_(shownState)
{
    WLOGFI("[NAPI] JsTransitionContext ctorCnt: %{public}d", ++ctorCnt1);
}

JsTransitionContext::~JsTransitionContext()
{
    WLOGFI("[NAPI] ~JsTransitionContext dtorCnt: %{public}d", ++dtorCnt1);
}
...
NativeValue* JsTransitionContext::OnCompleteTransition(NativeEngine& engine, NativeCallbackInfo& info)
{
    ...
    AsyncTask::CompleteCallback complete =
        [weakWindow = windowToken_, errCode, transitionCompleted, shownState = shownState_](
...
static NativeValue* CreateJsTransitionContextObject(NativeEngine& engine, std::shared_ptr<NativeReference> jsWin,
    sptr<Window> window, bool shownState)
{
    ...
    std::unique_ptr<JsTransitionContext> jsTransContext = std::make_unique<JsTransitionContext>(window, shownState);
...
{
    WLOGFI("[NAPI] JsTransitionController ctorCnt: %{public}d", ++ctorCnt2);
}

JsTransitionController::~JsTransitionController()
{
    WLOGFI("[NAPI] ~JsTransitionController dtorCnt: %{public}d", ++dtorCnt2);
}
...
void JsTransitionController::AnimationForShown()
{
    ...
    std::unique_ptr<AsyncTask::CompleteCallback> complete = std::make_unique<AsyncTask::CompleteCallback> (
        [self = weakRef_](NativeEngine&, AsyncTask&, int32_t) {
            ...
            // todo: updateRSTree in complete
            window->UpdateRsTree(true);
        }
    );
