                         ...
                         std::unique_ptr<ParamT>& param)
{
    ...
    if (info->ref == nullptr) {
        GNAPI_LOG("yqj info->ref == nullptr");
    ...
    auto completeFunc = [](napi_env env, napi_status status, void *data) {
        ...
        if (info->resolve) {
            GNAPI_LOG("yqj info->resolve != nullptr");
