...
Session::Session(const AbilityInfo& info) : abilityInfo_(info)
{
...
const AbilityInfo& Session::GetAbilityInfo() const
{
    return abilityInfo_;
}
...
WSError Session::Connect(const sptr<ISessionStage>& sessionStage)
{
...
WSError Session::StartAbility(const AbilityInfo& info)
{
    return WSError::WS_OK;
}
