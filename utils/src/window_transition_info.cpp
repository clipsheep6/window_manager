...
namespace OHOS {
namespace Rosen {
WindowTransitionInfo::WindowTransitionInfo(sptr<AAFwk::WindowTransitionInfo> info)
{
    bundleName_ = info->bundleName_;
    abilityName_ = info->abilityName_;
    mode_ = static_cast<WindowMode>(info->mode_);
    windowRect_ = *(reinterpret_cast<Rect*>(&(info->windowRect_)));
    abilityToken_ = info->abilityToken_;
    displayId_ = info->displayId_;
    windowType_ = static_cast<WindowType>(info->windowType_);
