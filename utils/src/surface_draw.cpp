...
bool SurfaceDraw::DrawImageRect(std::shared_ptr<RSSurfaceNode> surfaceNode, Rect rect,
    sptr<Media::PixelMap> pixelMap, uint32_t color, bool notFillWindow)
{
    ...
    if (!DoDrawImageRect(buffer, rect, pixelMap, color, notFillWindow)) {
...
bool SurfaceDraw::DoDrawImageRect(sptr<OHOS::SurfaceBuffer> buffer, const Rect rect, sptr<Media::PixelMap> pixelMap,
    uint32_t color, bool notFillWindow)
{
    int32_t winWidth = static_cast<int32_t>(rect.width_);
    int32_t winHeight = static_cast<int32_t>(rect.height_);
